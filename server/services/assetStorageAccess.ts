...
const ASSET_WRITE_PERMISSIONS: Record<string, string[]> = {
  ...
  [STORAGE_KEYS.ASSET_MATRIX_PUBLISH_TASKS]: [PERMISSION_KEYS.ASSETS, PERMISSION_KEYS.ASSETS_MATRIX_PUBLISH, PERMISSION_KEYS.MARKETING_PUBLISH],
};
...
export function canReadStorageKey(user: AuthenticatedUser, key: string): boolean {
  ...
  if (key === STORAGE_KEYS.ASSET_MATRIX_PUBLISH_TASKS) {
    return hasAnyExactPermission(user, ASSET_WRITE_PERMISSIONS[key], 'read');
  }
  if (key === STORAGE_KEYS.ASSET_INTERNET_ACCOUNTS) {
