...
import { DEFAULT_PAGE_SIZE, PRODUCT_LIST, STORAGE_KEYS } from '../shared/utils/constants';
  ...
  const opportunities = leads
    ...
    .map((lead, index): Opportunity => {
      ...
      return {
        ...
        lifecycleStatus: stage === '赢单' ? '已转订单' : '商机跟进中',
...
function normalizeOpportunity(opportunity: Opportunity): Opportunity {
  ...
  if (!opportunity.lifecycleStatus) {
    return {
      ...opportunity,
      lifecycleStatus: opportunity.status === '已转订单' ? '已转订单' : opportunity.status === '已退款' ? '已退款' : opportunity.status === '输单' ? '已流失' : '商机跟进中',
    };
  }
  return opportunity;
}
...
async function createFromLead(lead: Lead): Promise<ApiResponse<Opportunity>> {
  ...
  syncLeadLifecycleByLeadId(lead.id, '商机跟进中', { opportunityId: opportunity.id });
  return createSuccessResponse(opportunity);
}

function createOrderFromOpportunity(opportunity: Opportunity): { orderId: string; orderNo: string } {
  ...
  const order: Order = {
    ...
    changeHistory: [{
      ...
      action: 'create',
      operator: opportunity.ownerName || '系统',
      changedAt: now,
      summary: '商机赢单自动生成订单',
    }],
  };
...
async function updateStage(id: string, stage: OpportunityStage, lostReason?: string): Promise<ApiResponse<Opportunity | null>> {
  ...
  const orderLink = stage === '赢单' ? createOrderFromOpportunity(opportunity) : undefined;
  ...
  opportunity.lifecycleStatus = stage === '赢单' ? '已转订单' : stage === '输单' ? '已流失' : '商机跟进中';
...
async function reopenOpportunity(id: string): Promise<ApiResponse<Opportunity | null>> {
  ...
  opportunity.lifecycleStatus = '商机跟进中';
...
async function addFollowUp(id: string, content: string, createdBy = '当前用户'): Promise<ApiResponse<Opportunity | null>> {
  ensureInit();
  await delay(100);
  const opportunities = getStorageData<Opportunity[]>(STORAGE_KEYS.OPPORTUNITIES) || [];
  const opportunity = opportunities.find((item) => item.id === id);
