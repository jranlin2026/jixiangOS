...
app.use('/api/customer-data-exchange', createCustomerDataExchangeRouter({
  ...
  service: {
    ...
    precheckImport: async (rows, destination) => { calls.push(`precheck:${destination}:${rows[0]?.previousOwnerName}:${rows[0]?.firstOwnerName}:${rows[0]?.lastFollowUpRecord}:${rows[0]?.remark}`); return { readyCount: rows.length }; },
    confirmImport: async ({ rows, destination }) => { calls.push(`confirm:${destination}:${rows[0]?.previousOwnerName}:${rows[0]?.firstOwnerName}:${rows[0]?.lastFollowUpRecord}:${rows[0]?.remark}`); return { successCount: rows.length }; },
...
try {
  const row = { rowNumber: 2, name: '张三', phone: '13800000000', previousOwnerName: '销售乙', firstOwnerName: '销售甲', lastFollowUpRecord: '已确认报价', remark: '重点客户' };
  ...
  assert.deepEqual(calls, [
    'precheck:public_pool:销售乙:销售甲:已确认报价:重点客户',
    'confirm:public_pool:销售乙:销售甲:已确认报价:重点客户',
    'export:ids',
  ]);
} finally {
