...
assert.match(source, /const renderPhoneDetailSections[\s\S]*?renderPhoneSummaryCard\(phone\)[\s\S]*?renderPhoneRelationshipOverview\(phone\)[\s\S]*?renderPhoneIdentityCard\(phone\)[\s\S]*?renderPhoneOwnershipCard\(phone\)[\s\S]*?renderPhonePlanCard\(phone\)/, '手机号详情应优先展示关系，再按业务语义分区');
