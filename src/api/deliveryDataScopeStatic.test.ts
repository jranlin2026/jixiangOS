...
const permissionCatalog = readFileSync(join(root, 'src/pages/Settings/corePermissionCatalog.ts'), 'utf8');
...
assert.match(rolePermission, /getCoreDataScopeRows/, '角色配置页必须使用核心数据范围清单');
assert.match(permissionCatalog, /domain: 'deliveries', label: '交付数据'/, '核心数据范围清单必须显示交付数据范围');
