...
const workbenchSchedulerActor = {
  id: 'system-workbench-scheduler',
  name: '系统工作台调度器',
  account: 'system-workbench-scheduler',
  email: '',
  phone: '',
  role: '系统管理员',
...
const workbenchScheduler = createWorkbenchScheduler({
  store: createPrismaSchedulerStore(prisma as any),
  generateDailyTasks: async ({ date, signal, lease }) => {
    const result = await enterpriseTaskService.generateDailyTasks(date, workbenchSchedulerActor, { signal, lease });
...
app.get('/api/customers/lead-source-facets', requireCustomerReadAccess, async (req: AuthenticatedRequest, res) => {
  const scope = queryParam(req.query.scope) === 'public_pool' ? 'public_pool' : 'active';
  const result = await customerListService.listLeadSourceFacets(scope, req.currentUser);
  res.status(result.code === 0 ? 200 : 400).json(result);
});
...
app.get('/api/refunds', requireStorageAccess, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await refundQueryService.list({
    ...
    refundCategory: text(req.query.refundCategory) as any,
    owner: text(req.query.owner),
    productLevel: text(req.query.productLevel),
    minAmount: req.query.minAmount === undefined ? undefined : Number(req.query.minAmount),
    maxAmount: req.query.maxAmount === undefined ? undefined : Number(req.query.maxAmount),
...
app.get('/api/assets/:kind', requireAssetOrMarketingPublishReadAccess, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await assetListService.list(kind, {
    ...
    status: queryParam(req.query.status),
    deviceCategory: queryParam(req.query.deviceCategory),
    brand: queryParam(req.query.brand),
    communicationType: queryParam(req.query.communicationType),
    acquisitionType: queryParam(req.query.acquisitionType),
    profileStatus: queryParam(req.query.profileStatus) as AssetFilters['profileStatus'],
    operator: queryParam(req.query.operator),
    attributionLocation: queryParam(req.query.attributionLocation),
    simForm: queryParam(req.query.simForm),
    accountCategory: queryParam(req.query.accountCategory),
    departmentId: queryParam(req.query.departmentId),
    ownerId: queryParam(req.query.ownerId),
    currentUserId: queryParam(req.query.currentUserId),
    userAssignment: queryParam(req.query.userAssignment) as AssetFilters['userAssignment'],
    phoneBinding: queryParam(req.query.phoneBinding) as AssetFilters['phoneBinding'],
    deviceBinding: queryParam(req.query.deviceBinding) as AssetFilters['deviceBinding'],
    loginDeviceBinding: queryParam(req.query.loginDeviceBinding) as AssetFilters['loginDeviceBinding'],
    accountBinding: queryParam(req.query.accountBinding) as AssetFilters['accountBinding'],
    identityBinding: queryParam(req.query.identityBinding) as AssetFilters['identityBinding'],
    credentialStatus: queryParam(req.query.credentialStatus) as AssetFilters['credentialStatus'],
    twoFactorStatus: queryParam(req.query.twoFactorStatus) as AssetFilters['twoFactorStatus'],
    servicePasswordStatus: queryParam(req.query.servicePasswordStatus) as AssetFilters['servicePasswordStatus'],
    packageName: queryParam(req.query.packageName),
    contractStatus: queryParam(req.query.contractStatus) as AssetFilters['contractStatus'],
    monthlyFeeMin: queryParam(req.query.monthlyFeeMin) ? Number(queryParam(req.query.monthlyFeeMin)) : undefined,
    monthlyFeeMax: queryParam(req.query.monthlyFeeMax) ? Number(queryParam(req.query.monthlyFeeMax)) : undefined,
    page: Number(queryParam(req.query.page) || 1),
...
async function startServer() {
  ...
  if (setupStatus.data.initialized) workbenchScheduler.start();
