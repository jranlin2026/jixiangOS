...
import { getUserRole, isSuperAdminUser, PERMISSION_KEYS, roleHasPermission } from '../shared/utils/permissions';
...
function isCurrentUserSuperAdmin(): boolean {
  ...
  return isSuperAdminUser(user, readJson<Role[]>(STORAGE_KEYS.ROLES) || []);
}
...
function filterVisibleApplications(applications: OrderApplication[]): OrderApplication[] {
  applications = applications.filter((application) => !application.reviewCleanedAt);
...
async function cleanupDeletedSourceOrderApplication(id: string, reason: string): Promise<ApiResponse<boolean>> {
  ...
  saveApplications(applications.map((item) => item.id === id ? {
    ...
    reviewCleanedBy: getCurrentUser()?.name || '超级管理员',
    reviewCleanupReason: normalizedReason,
    updatedAt: cleanedAt,
  } : item));
  return createSuccessResponse(true);
}
