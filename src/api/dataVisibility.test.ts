...
assert.deepEqual(managerScope.orders, ['order-b', 'order-a'], '订单默认按最新创建排序，时间相同时按 ID 倒序');
...
assert.deepEqual(aaScope.orders, ['order-other', 'order-b', 'order-a']);
...
assert.deepEqual(adminScope.orders, ['order-other', 'order-b', 'order-a']);
...
assert.deepEqual(systemAdminScope.orders, ['order-other', 'order-b', 'order-a']);
