...
function buildOrder(
  ...
  return {
    ...
    ...(application.importBatchId ? {
      importBatchId: application.importBatchId,
      importRowNumber: application.importRowNumber,
      importedById: application.importedById,
      importedByName: application.importedByName,
      importedAt: application.importedAt,
      targetCreatorId: application.targetCreatorId,
      targetCreatorName: application.targetCreatorName,
      importWarnings: application.importWarnings ? [...application.importWarnings] : undefined,
    } : {}),
      ...
      for (let attempt = 1; attempt <= MAX_TRANSACTION_ATTEMPTS; attempt += 1) {
        try {
          const cleaned = await prisma.$transaction(async (transaction) => {
            ...
            const cleanedApplication: OrderApplication = {
              ...application,
            ...
            await transaction.businessRecord.update({
              ...
              data: {
                eventAt: new Date(cleanedAt),
                data: cleanedApplication as unknown as Prisma.InputJsonValue,
              },
            });
            return true;
          }, {
