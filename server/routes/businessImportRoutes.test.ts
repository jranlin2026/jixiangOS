...
try {
  ...
  assert.equal((await fetch(`${base}/orders/precheck`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ rows: [row, { ...row, thirdPartyOrderNo: 'TP-2' }] }) })).status, 400, 'DTO rejects duplicate rowNumber before precheck persistence');
