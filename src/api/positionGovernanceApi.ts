...
export const positionGovernanceApi = {
  ...
  getReconciliation(id: string, filters: { page?: number; pageSize?: number } = {}) {
    if (!shouldUseBackendApi()) return Promise.resolve(backendRequired<PositionGovernanceReconciliation>());
    const query = new URLSearchParams();
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined) query.set(key, String(value));
    });
    return backendRequest<PositionGovernanceReconciliation>(`/settings/position-governance/batches/${encodeURIComponent(id)}/reconciliation?${query.toString()}`);
  },
