...
## 2026-07-16 业务附件与售后挽回第一批

| 编号 | 等级 | 问题描述 | 修改与验证 | 剩余风险 |
|---|---|---|---|---|
| ATTACHMENT-001 | P1 | 订单、售后和交付凭证只能单张或仅保存文件名，无法可靠查看与追溯 | 新增非公开业务附件存储及鉴权读写；订单付款限 1 张，其他凭证/交付附件限 8 张；支持多选、粘贴、下载、删除与重传。177 个测试文件及生产构建通过 | 未使用真实 QA 账号执行多角色浏览器冒烟 |
| RECOVERY-001 | P1 | 售后挽回来源为自由文本，联系方式可全部留空 | 新增平台/店铺配置与联动选择；服务端复用客户规则，强制姓名且手机号/微信至少一项 | 历史自由文本来源继续按名称兼容展示，不自动映射配置 ID |
| DELIVERY-001 | P1 | 交付步骤填写内容只有勾选完成时保存，容易误关丢失 | 增加步骤独立保存、保存成功时间、未保存状态和关闭确认；列表新增实际完成时间，步骤显示完成人与完成时间 | 本轮未增加交付模板和发票能力，仍按已确认范围排除 |
| ATTACHMENT-SEC-001 | P0 | 业务对象若直接信任客户端附件元数据，可能夹带内部存储字段 | 订单、售后、交付持久化改为字段白名单；列表投影二次剥离存储名、缓冲区和内联数据 | 真实环境文件目录权限仍需部署后核验 |
| ATTACHMENT-002 | P1 | 功能分支位于 `.worktrees` 时附件已落盘但查看返回 HTML 404 | 已鉴权附件读取显式允许受控隐藏父目录；原附件接口实测返回 200、正确图片类型和完整字节数，并增加路由回归约束 | 附件目录仍必须保持非公开且只允许通过鉴权路由访问 |
