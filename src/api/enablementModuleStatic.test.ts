...
assert.match(home, /未解锁/);
assert.match(home, /今天的任务/);
assert.match(home, /AI导师/);
assert.match(home, /管理视角/);
assert.match(home, /体验AI导师/);
assert.match(home, /mentorDemoMessage/);
assert.doesNotMatch(home, /共5项待办行动/);
assert.match(home, /onOpenKnowledge/);
assert.match(home, /useSearchParams/);
assert.match(home, /resolveEnablementHomeView\(searchParams\.get\('view'\), canManage\)/);
assert.match(home, /setEnablementSearchParam\(searchParams, 'view', next\)/);
assert.match(page, /setEnablementSearchParam\(searchParams, 'tab', value\)/);
