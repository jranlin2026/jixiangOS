...
assert.match(dialogSource, /同步缺失来源/);
assert.match(dialogSource, /同步缺失标签/);
assert.match(dialogSource, /customerDataExchangeApi\.syncImportConfigs/);
assert.match(dialogSource, /customerDataExchangeApi\.precheckImport\(rows, destination\)/);
