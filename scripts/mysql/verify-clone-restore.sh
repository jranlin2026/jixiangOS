...
if [[ "$MIGRATION_COUNT" == "0" || "$FAILED_MIGRATION_COUNT" != "0" ]]; then
  echo "Clone restore verification failed: migration history is incomplete" >&2
  exit 1
fi

printf 'Clone restore verification passed: database=%s tables=%s migrations=%s failed_migrations=%s\n' \
  "$MYSQL_DATABASE" "$TABLE_COUNT" "$MIGRATION_COUNT" "$FAILED_MIGRATION_COUNT"
