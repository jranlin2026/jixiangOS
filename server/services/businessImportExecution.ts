    ...
    async execute(job, row) {
      ...
      const result = await deps.createImportedRecoveryOrder({
        ...
        data: {
          ...
          originalAmount: number(input.originalAmount), originalPaymentAt: clean(input.originalPaymentAt) || undefined,
          recoveryAmount: number(input.recoveryAmount), recoveryAt: clean(input.recoveryAt),
