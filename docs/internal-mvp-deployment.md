...
- 客户、订单、交付、退款、分账、升单、产品、标签等数组型业务数据已批量升级到结构化表 `business_records`。
...
- 客户、订单、交付、退款、分账、升单、产品、标签等数组型业务数据存入 `business_records`，按业务域 `domain` 区分，并提取标题、状态、负责人、客户ID、订单ID、金额和业务时间等索引字段。
- 非数组配置类数据继续保留在 `app_storage`，例如产品等级、订单类型、生命周期配置、财务总览对象等。
- 这是第二阶段到第三阶段之间的批量结构化架构，优先解决多人共享、统一持久化、可索引查询和可继续拆表的问题。
