...
- 首屏优先展示状态、销售、今日跟进客户、风险客户和需要介入；目标、实收、完成率及客户数在同一张统一表内。
...
2. 将“今日跟进客户”改为当天真实新增的客户跟进记录，并按员工去重客户数；待办完成数不再伪装成有效跟进。
...
# 老板驾驶舱穿透式管理 V1 第二阶段设计 QA

## 对照范围

- 销售个人经营参考图：`/var/folders/x4/fnz851dj7rv2p9y0_1zx4gx40000gn/T/codex-clipboard-dbdf8495-5a1c-4e08-9549-38d325f03f44.png`。
- 客户穿透详情参考图：`/var/folders/x4/fnz851dj7rv2p9y0_1zx4gx40000gn/T/codex-clipboard-dca3ee65-e7cd-4fbf-ab3b-4f59a7a9ad7c.png`。
- 实现页面：`/management/sales/:salespersonId` 与现有客户详情弹窗。
- 本轮不操作用户当前浏览器，视觉验收依据为参考图、现有设计令牌、响应式结构与生产构建；未伪造新的浏览器截图。

## 高保真界面

- 个人经营页按参考图保留“一句总结 → 4 张指标卡 → 业绩/阶段 → 客户表”的单任务层级。
- 客户详情按参考图增加横向真实沟通节点、无沟通空窗提醒和单张经营决策摘要。
- 介入任务使用现有任务中心状态与权限，不引入第二套按钮、表格或状态色。
- 所有节点、风险、阶段、下一步动作和任务状态均来自真实业务数据，未使用示例名称、硬编价格或伪 AI 结论。

## 交互与响应式检查

- 销售战情 → 个人经营 → 客户详情为 3 次以内穿透路径。
- 个人客户表桌面端使用统一表格，移动端使用同语义卡片，总数、页码、每页条数和跳页复用系统分页。
- 老板可下达“提醒销售 / 主管协同 / 老板亲跟”，员工可在客户页提交结果，管理者可验收或退回。
- `npm run build` 通过；销售个人页作为独立懒加载 chunk 生成，未拉高首页初始包。

## 最终结果

代码与交互验收通过。新页面继续使用极享OS现有白底、浅灰背景、紫色主操作和统一数据表规范；没有新的 P0/P1 规范偏离。
